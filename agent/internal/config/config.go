...
func Load() (Config, error) {
    ...
    flag.Func("controller-ca-pins", "Comma-separated SHA256 fingerprints for pinning controller certificates", func(val string) error { if val == "" { cfg.ControllerCAPins = nil; return nil }; cfg.ControllerCAPins = strings.Split(val, ","); return nil })
    if cfg.ControllerCAPins == nil { cfg.ControllerCAPins = defaultPins }
    ...
    flag.Func("registry-allowlist", "Comma separated list of registries permitted for deployments", func(val string) error { if val == "" { cfg.RegistryAllowList = nil; return nil }; cfg.RegistryAllowList = strings.Split(val, ","); return nil })
    if cfg.RegistryAllowList == nil { cfg.RegistryAllowList = defaultRegistries }
    ...
    flag.Func("allowed-volume-roots", "Comma separated host paths that volume mounts must fall under", func(val string) error { if val == "" { cfg.AllowedVolumeRoots = nil; return nil }; cfg.AllowedVolumeRoots = strings.Split(val, ","); return nil })
    if cfg.AllowedVolumeRoots == nil { cfg.AllowedVolumeRoots = defaultVolumeRoots }
    ...
    flag.Parse()
    if cfg.ControllerURL == "" { return Config{}, errors.New("controller URL is required") }
    if cfg.AgentToken == "" { return Config{}, errors.New("agent token is required") }
    if cfg.WorkDir == "" { cfg.WorkDir = fmt.Sprintf("%s/work", cfg.DataDir) }
    return cfg, nil
}

func durationFromEnv(key string, fallback time.Duration) time.Duration { val := os.Getenv(key); if val == "" { return fallback }; n, err := strconv.Atoi(val); if err != nil { return fallback }; return time.Duration(n) * time.Second }
func valueOr(v, fallback string) string { if v == "" { return fallback }; return v }
func boolFromEnv(key string, def bool) bool { val := os.Getenv(key); if val == "" { return def }; b, err := strconv.ParseBool(val); if err != nil { return def }; return b }
func listFromEnv(key string) []string { val := strings.TrimSpace(os.Getenv(key)); if val == "" { return nil }; parts := strings.Split(val, ","); out := make([]string, 0, len(parts)); for _, p := range parts { trimmed := strings.TrimSpace(p); if trimmed != "" { out = append(out, trimmed) } }; if len(out) == 0 { return nil }; return out }
