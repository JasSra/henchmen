"""
In-memory job queue with SQLite persistence for recovery (moved under controller/app)
"""
import asyncio
import uuid
...
class JobQueue:
	...
	async def enqueue(self, job_create: JobCreate) -> Job:
		...
		async with self._lock:
			...
			if job_create.deployment is not None:
				if hasattr(job_create.deployment, "model_dump"):
					deployment_spec = job_create.deployment.model_dump(mode="json")
				else:
					deployment_spec = job_create.deployment
			...
			job = Job(
				...
				metadata=dict(job_create.metadata or {}),
				deployment_id=job_create.deployment_id,
