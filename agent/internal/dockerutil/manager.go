...
func (m *Manager) Restart(ctx context.Context, name string) error {
	return m.cli.ContainerRestart(ctx, name, container.StopOptions{Timeout: nil})
}

// Stop stops a container by name or ID.
func (m *Manager) Stop(ctx context.Context, name string) error {
	to := container.StopOptions{Timeout: nil}
	return m.cli.ContainerStop(ctx, name, to)
}
...
func (m *Manager) Logs(ctx context.Context, containerName string, tail int, follow bool) (io.ReadCloser, error) {
	opt := container.LogsOptions{ShowStdout: true, ShowStderr: true, Follow: follow, Tail: fmt.Sprintf("%d", tail)}
	return m.cli.ContainerLogs(ctx, containerName, opt)
}
...
func (m *Manager) FindContainerByLabel(ctx context.Context, key, value string) ([]types.Container, error) {
	...
	return m.cli.ContainerList(ctx, container.ListOptions{All: true, Filters: args})
}
