...
func WithTLSConfig(cfg TLSConfig) Option { return func(o *clientOptions) { o.tls = cfg } }

// WithSecurityBypass relaxes strict enforcement in trusted environments.
func WithSecurityBypass() Option { return func(o *clientOptions) { o.securityBypass = true } }
...
func New(base, token string, opts ...Option) (*Client, error) {
	...
	return &Client{ baseURL: u, httpClient: &http.Client{Timeout: 30 * time.Second, Transport: transport}, agentToken: token }, nil
}

func buildTLSConfig(serverName string, opts clientOptions) (*tls.Config, error) {
	tlsCfg := &tls.Config{MinVersion: tls.VersionTLS12}
	if serverName != "" { tlsCfg.ServerName = serverName }
	if opts.tls.AllowInsecure || opts.securityBypass { tlsCfg.InsecureSkipVerify = true }
	if opts.tls.CAFile != "" {
		data, err := os.ReadFile(opts.tls.CAFile)
		if err != nil { return nil, fmt.Errorf("failed to read controller CA: %w", err) }
		pool := x509.NewCertPool()
		if !pool.AppendCertsFromPEM(data) { return nil, errors.New("controller CA file is invalid") }
		tlsCfg.RootCAs = pool
	}
	if opts.tls.ClientCert != "" || opts.tls.ClientKey != "" {
		if opts.tls.ClientCert == "" || opts.tls.ClientKey == "" { return nil, errors.New("client cert and key must both be provided") }
		cert, err := tls.LoadX509KeyPair(opts.tls.ClientCert, opts.tls.ClientKey)
		if err != nil { return nil, fmt.Errorf("failed to load client certificate: %w", err) }
	...
	if len(pins) > 0 {
		tlsCfg.VerifyPeerCertificate = func(rawCerts [][]byte, _ [][]*x509.Certificate) error {
			for _, der := range rawCerts {
				...
				for _, pin := range pins { if bytes.Equal(hash[:], pin) { return nil } }
			}
...
func normalizePins(values []string) ([][]byte, error) {
	...
	for _, v := range values {
		...
		if err != nil { return nil, fmt.Errorf("invalid certificate pin %q: %w", v, err) }
...
// Job is a minimal job representation returned by the controller.
type Job struct {
	ID      string          `json:"id"`
...
func (c *Client) Register(ctx context.Context, req RegisterRequest) (RegisterResponse, error) {
	...
	if err := c.do(ctx, http.MethodPost, "/v1/agents/register", req, &resp); err != nil { return RegisterResponse{}, err }
...
func (c *Client) Heartbeat(ctx context.Context, agentID string, req HeartbeatRequest) (HeartbeatResponse, error) {
	...
	if err := c.do(ctx, http.MethodPost, endpoint, req, &resp); err != nil { return HeartbeatResponse{}, err }
	return resp, nil
}
...
func (c *Client) StreamLogs(ctx context.Context, agentID, jobID string, reader io.Reader) error {
	...
	if c.agentToken != "" { req.Header.Set("Authorization", "Bearer "+c.agentToken) }
