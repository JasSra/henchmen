...
type Job struct { ID string `json:"id"`; Type JobType `json:"type"`; Payload json.RawMessage `json:"payload"` }
...
type HealthCheckSpec struct { Type string `json:"type"`; Endpoint string `json:"endpoint"`; ExpectedStatus int `json:"expected_status"` }

// VolumeMapping maps a host path to a container path.
type VolumeMapping struct { Source string `json:"source"`; Target string `json:"target"` }

// PortMapping allows explicit or automatic port assignments.
type PortMapping struct { Key string `json:"key"`; Target int `json:"target"`; Published string `json:"published"`; Protocol string `json:"protocol"` }

// ContainerJobPayload describes container selection actions.
type ContainerJobPayload struct { Name string `json:"name"`; Container string `json:"container"` }

// LogsJobPayload carries log streaming parameters.
type LogsJobPayload struct { Name string `json:"name"`; Container string `json:"container"`; Tail int `json:"tail"`; FollowMins int `json:"follow_minutes"` }

// ExecJobPayload describes command execution requests on the host.
type ExecJobPayload struct { Command []string `json:"command"`; Environment map[string]string `json:"environment"`; TimeoutSeconds int `json:"timeout_seconds"`; WorkingDir string `json:"working_dir"` }

// EnvQueryPayload requests specific environment variables from the agent host.
type EnvQueryPayload struct { Keys []string `json:"keys"` }
